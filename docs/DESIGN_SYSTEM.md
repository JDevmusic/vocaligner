...
Superseded description (kept for history): the landing page hero gradient originally ran white at the top, through a warm sunset gold, down into a deep purple base — explored and rejected at the time: pink/red/coral as part of that gradient, and a fully-saturated "vivid" version of the same gradient edge-to-edge, both read as generic AI-product styling rather than premium.

v1.36 (2026-08-21) revised that hero's colour balance and CTA form, per founder feedback that the landing page didn't read as premium enough. The gradient held flat white/near-white for the top ~46% of the hero (headline + subhead) before transitioning into the sunset-to-purple wash lower down, where the CTA and chain-preview footer sat — same final colors and proportions as before, just compressed into the bottom half instead of starting immediately at 0%. This followed the "colour as one confident moment, not a wash" principle already named under Design Inspiration (ElevenLabs) but under-applied in the original full-height gradient. Separately, the hero CTA's "Match [artist] on [song], in Logic Pro" sentence-style form was replaced with a "See For Yourself" heading above two clearly labeled Artist/Song fields side by side — a more conventional SaaS pattern, founder-requested; this labeled-field pattern is still current (see the Hero Section spec below), even though v1.36's own colour treatment was later superseded.

v1.37 replaced v1.36's colour treatment with a dark navy hero (deepening twice, toward a "space" feel) blending into purple lower down — itself superseded by v1.38 above.
