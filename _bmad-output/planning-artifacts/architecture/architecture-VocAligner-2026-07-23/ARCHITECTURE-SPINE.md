...
### AD-6 — Design tokens are centralized `[ADOPTED, tightened]`
...
- **Rule:** All color/gradient values are defined once as CSS custom properties in `globals.css` (wired through Tailwind's `@theme inline`). Components consume them only via the semantic Tailwind classes built on those tokens (`text-foreground`, `text-muted`, `text-supporting`, `bg-background`, `hero-gradient`, and any token added the same way) — **never a raw Tailwind palette utility either** (`zinc-*`, `black/`, `white`, etc.), not just never a raw hex value. A new visual need is met by adding a `--color-*` token to `globals.css` first, then a semantic class for it — not by reaching for Tailwind's built-in palette directly. *(Existing pages already use un-tokenized palette utilities like `border-black/10`, `bg-zinc-800` — see Deferred; this Rule governs new work, cleanup of existing pages is separate.)*
...
- **Bespoke per-plugin Plugin Visual designs.** MVP reuses one default visual treatment (PRD §4.3); per-plugin-type visuals are future work, not architecturally blocked by anything here.
