...
- **Widened 2026-08-03, reverted 2026-08-13**: temporarily also permitted `app/api/compare/route.ts` — an internal, unlinked tool for comparing model quality/cost side by side. Decommissioned once the founder settled on Luna as the production model (the question that tool existed to answer); code review of Epic 5's rate-limiting story separately flagged it as an unprotected, uncached, multi-model AI-cost exposure, which removing it also resolves. `app/compare/`, `app/api/compare/route.ts`, and `lib/ai/comparisonModels.ts` were deleted entirely — `lib/ai/openRouterModelClient.ts` was NOT touched, since Luna's production path (`getModelClient.ts`/`failoverModelClient.ts`) depends on it independently of the removed comparison tool.
...
- ~~**Authentication / accounts.**~~ `[MOVED IN-SCOPE 2026-08-22]` No longer deferred — founder-confirmed decision to build Epic 6 (Accounts + Saved Songs), see `_bmad-output/planning-artifacts/epics.md`. Supabase (Postgres + Auth) is the new boundary for user identity and user-owned data, additive alongside the existing Upstash-backed generation store — not a replacement of it. AD-7's cache stays global exactly as adopted; a per-user concept now exists (Supabase) but only for the new, separate "saved songs" reference layer, not the anonymous Artist+Song cache itself. A formal AD-13 (and an explicit AD-7 amendment note) are tracked as candidates in `epics.md`'s Additional Requirements pending Epic 6 actually shipping, rather than ratified here ahead of the code existing — same convention as AD-11/AD-12 before Epics 4/5 landed.
