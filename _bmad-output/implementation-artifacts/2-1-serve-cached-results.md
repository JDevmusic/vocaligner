...
- [x] [Review][Decision] Should `getCachedGeneration` also gate on `schemaVersion`? — **Resolved: yes.** Founder chose option (b): added `entry.meta.schemaVersion !== CURRENT_SCHEMA_VERSION` to the mismatch check as an explicit second safety net, rather than relying on the (undocumented-in-code) convention that a schema change always bumps `PIPELINE_VERSION` too. Architecture AD-8 updated to specify the 4-tuple `(Artist+Song key, PIPELINE_VERSION, PROMPT_VERSION, CURRENT_SCHEMA_VERSION)`. New test: "treats a schemaVersion mismatch as a cache miss". [web/lib/store/generationStore.ts, `_bmad-output/planning-artifacts/architecture/architecture-VocAligner-2026-07-23/ARCHITECTURE-SPINE.md` AD-8]
- [x] [Review][Decision] Should cache-key normalization widen beyond "trim + lowercase"? — **Resolved: yes.** Founder chose to widen it: added `.normalize("NFC")` and internal-whitespace collapsing (`.replace(/\s+/g, " ")`) to `normalizeCacheKey` (extracted into a `normalizeText` helper). Confirmed this doesn't cross into "fuzzy matching" (still excluded) — it only collapses text that's genuinely identical, just encoded or spaced differently. Architecture AD-7 updated to specify `normalize` precisely. Two new tests: NFC vs. NFD accented text (built from explicit `é`/`́` escapes, not typed literally, so the two forms can't accidentally end up identical) and internal-whitespace-width matching. [web/lib/store/generationStore.ts, `_bmad-output/planning-artifacts/architecture/architecture-VocAligner-2026-07-23/ARCHITECTURE-SPINE.md` AD-7]
