...
- [x] Task 1: Build the 8 knob-based bespoke visuals (AC: 1, 2, 5)
  ...
  - [x] **Full real-panel structure, not just the controls with data.** First pass only rendered the handful of controls the registry has data for, as a bare single row — a regression to the generic-template approach the bespoke direction was meant to replace. Fixed: every real section/control/meter/mode-tab from each plugin's reference screenshot now has its real position on the panel, shown faded (`FadedKnob`/`FadedField`/`FadedTabs`/`FadedDisplay` in `PluginKnobPrimitives.tsx`) with Logic's own real default/resting value where there's no backing data — e.g. Compressor's circuit-mode tabs, gain-reduction meter, Limiter section, and Input/Output Gain; ChromaVerb's Damping EQ curve and Attack/Size/Density/Distance/Dry row; Tape Delay's entire Character and Modulation sections. Verified against each plugin's own `docs/images.md/*_plugin.png` individually. Chorus's real panel genuinely has only 3 knobs (verified directly) — no faded elements needed there, left as-is.
