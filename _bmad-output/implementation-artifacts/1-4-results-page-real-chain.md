...
- [x] [Review][Decision] Primary navigation actions lost native `<a>` semantics when converted to `AnimatedButton` — resolved: founder chose option (b). `AnimatedButton` now accepts an `href` prop (mutually exclusive with `onClick`/`type`/`disabled`/`form` via a discriminated union) and renders a real anchor for that variant, keeping the same hover/tap micro-interaction. First attempt wrapped `next/link`'s `Link` with `motion.create()`, but Motion couldn't get a working DOM ref through it — `whileHover`/`whileTap` silently did nothing, caught by checking the rendered element's actual style in a browser rather than trusting the typecheck. Switched to a plain `motion.a` (Motion's own primitive, so the animation is guaranteed to work); the only cost is a full page reload instead of a client-side transition for these three "go back" actions. All three call sites (`loading/page.tsx`'s "Try again", `results/page.tsx`'s "Back to Home" and "Try Another Song") now pass `href="/"` instead of `onClick={() => router.push("/")}`; the now-unused `useRouter()` calls were removed from `NothingHereState`/`ResultsContent`. Verified in-browser via Playwright: both anchors render as real `<a href="/">` elements and show `transform: scale(1.035)` on hover, matching the landing page's own submit button. [web/app/components/AnimatedButton.tsx, web/app/loading/page.tsx, web/app/results/page.tsx]
